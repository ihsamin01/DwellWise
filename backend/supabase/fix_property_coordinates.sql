-- ---------------------------------------------------------------------------
-- Tighten every listing's map pin onto the real coordinates of the area named
-- in its address.
--
-- The seed originally scattered pins up to ~900 m from the area centre, which
-- put some of them outside the neighbourhood the address names. This pulls them
-- back to within ~100 m, so the embedded map (and "Open in Google Maps") lands
-- on the right place. The tiny jitter that remains just stops all ten listings
-- in an area from stacking on one identical point.
--
...
set latitude  = a.lat + (random() - 0.5) * 0.0018,
    longitude = a.lng + (random() - 0.5) * 0.0018
from areas a
where lower(p.area) = lower(a.area);

-- Check: how far each listing now sits from its area centre (metres).
with areas(area, city, lat, lng) as (
  values
    ('Dhaka','',23.8103,90.4125),
    ('Banani','Dhaka',23.7937,90.4066),
...
select
  round(avg(dist))::int as avg_offset_m,
  round(max(dist))::int as max_offset_m,
  count(*)              as listings
from (
  select sqrt(
    power((p.latitude - a.lat) * 111320, 2) +
    power((p.longitude - a.lng) * 111320 * cos(radians(p.latitude)), 2)
  ) as dist
  from public.properties p
  join areas a on lower(p.area) = lower(a.area)
) d;
