-- ---------------------------------------------------------------------------
-- Storage access for profile photos.
--
-- Lets a signed-in user upload their own profile photo (overwriting their
-- previous one) to the `avatars` bucket, and lets anyone read them, since
-- avatar URLs are shown publicly across the app (property cards, chat, etc).
--
-- Run in Supabase Dashboard -> SQL Editor. Safe to re-run.
-- ---------------------------------------------------------------------------

-- Make sure the bucket exists and is public (public URLs, no signed links).
insert into storage.buckets (id, name, public)
values ('avatars', 'avatars', true)
on conflict (id) do update set public = true;

drop policy if exists "avatars are publicly readable" on storage.objects;
drop policy if exists "authenticated can upload their own avatar" on storage.objects;
drop policy if exists "owners can update their own avatar" on storage.objects;
drop policy if exists "owners can delete their own avatar" on storage.objects;

create policy "avatars are publicly readable"
  on storage.objects for select
  using (bucket_id = 'avatars');

-- Each user's avatar lives under a folder named after their own uid
-- (see SupabaseService.uploadAvatar), so this also stops one user
-- overwriting another's photo.
create policy "authenticated can upload their own avatar"
  on storage.objects for insert to authenticated
  with check (bucket_id = 'avatars' and (storage.foldername(name))[1] = auth.uid()::text);

create policy "owners can update their own avatar"
  on storage.objects for update to authenticated
  using (bucket_id = 'avatars' and (storage.foldername(name))[1] = auth.uid()::text);

create policy "owners can delete their own avatar"
  on storage.objects for delete to authenticated
  using (bucket_id = 'avatars' and (storage.foldername(name))[1] = auth.uid()::text);

-- Check
select policyname
from pg_policies
where schemaname = 'storage'
  and tablename = 'objects'
  and policyname ilike '%avatar%'
order by policyname;
